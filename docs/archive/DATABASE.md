...
Every table enforces strict default denial (`ALTER TABLE <name> ENABLE ROW LEVEL SECURITY;`) paired with declarative role permissions matching verified operational boundaries:
- **`profiles`**: Readable by self (`auth.uid() = id`) and authenticated operational staff (`waiter`, `kitchen`, `cashier`, `manager`); write and update access restricted strictly to self or `manager`.
- **`menu_categories` / `menu_items`**: Public read for all dining guests and staff. Write access for `menu_categories` is restricted strictly to `manager`; write access for `menu_items` (including kitchen availability toggles) is restricted to `kitchen` and `manager`.
- **`inventory`**: Viewable by operational staff (`kitchen`, `waiter`, `cashier`, `manager`) to verify ingredient availability; write access restricted to `kitchen` (ingredient depletion) and `manager` (replenishment).
- **`tables`**: Public read access for guest QR session validation and menu landing; occupancy and table state updates restricted to `waiter`, `cashier`, and `manager`.
- **`reservations`**: Viewable and creatable by guests and operational staff; booking time modifications and status transitions (`CONFIRMED`, `SEATED`, `CANCELLED`) restricted to `waiter` (hosts) and `manager`.
- **`orders` / `order_items`**: Read and insert access enabled for guests with validated table QR session tokens and staff; order workflow updates restricted to `waiter`, `kitchen`, `cashier`, and `manager`.
- **`payments` / `analytics_daily`**: Financial ledger read/write over payments restricted strictly to `cashier` and `manager`; daily executive KPI analytics restricted strictly to `manager`.
- **`notifications` / `reviews`**: AI and operational notifications readable and updatable exclusively by assigned target staff role or `manager`; reviews publicly readable and creatable by dining guests.
